...
if [ $cmd = "create" ]; then
	android create project \
		...
		--package $pkg \
		--gradle \
		--gradle-version 1.1.3 #plugin version
	...
	sed -i 's/runProguard false/minifyEnabled true/' \
		$path/build.gradle
	sed -i 's/gradle-1.12-all.zip/gradle-2.2.1-all.zip/' \
		$path/gradle/wrapper/gradle-wrapper.properties
	cp script.sh $path/emulator.sh

elif [ $cmd = 'start' ]; then 
	emulator -use-system-libs -avd pcduino -no-boot-anim -scale 1.50 -show-kernel
elif [ $cmd = "build" ]; then 
	sh gradlew build
	...
	tar -C $(dirname $(pwd)) --exclude=$path/*.tar --exclude=$path/*.sh \
		--exclude=$path/build/intermediates/* -cvf $path.tar $path
